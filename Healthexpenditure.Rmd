...
### (1) What is this common philosphy? 
...
## 2. The `readr` R package for reading healthcare coverage and spending information 
...
### (1) Read in healthcare coverage data
...
### (2) Read in healthcare spending data
...
## 3. Load the State information: the `datasets` R package 
...
### (2) Convert healthcare coverage data to a tidy format
...
### (3) Convert back to a wide format
...
### (4) Convert healthcare spending data to a tidy format
...
### (2) How does `dplyr` R package compare to using base functions R?
...
### Data wrangling in the `coverage` dataset   

### (4) `dplyr` verbs in action: `separate()`, `unite()`, ...
...
### Data wrangling in the `spending` dataset      
...
### (5) Selecting columns using `select()`
...
### (7) Arrange or re-order rows using `arrange()` 
...
### (8) Join two datasets together using `left_join()`, `right_join()`, `inner_join()`, ... 
#### (8) a. Quick review/summary of loading data and data wrangling   
Now, based on what we've learned, we can read data and wrangle the data in just several lines! 
Let's see how clean the codes can be!
...
#### (8) b. Join two datasets together
...
### (10)  Create summaries of columns using `summarise()`
...
### (11) Group operations using `group_by()`
